	...
	<div>
		...
		<div class="pt-10">
			<div class="flex flex-row justify-between pb-5">
				<div class="font-display text-orange text-3xl flex-initial">
					My Events
				</div>
				<button
...
export default {
	...
	async created() {
		...
		bus.$on('event-created', this.addEvent);
		bus.$on('event-updated', this.updateEvent);
		bus.$on('event-deleted', this.removeEvent);
	},
	...
	methods: {
		...
		addEvent(newEvent) {
			this.$set(this.events, this.events.length, newEvent);
		},
